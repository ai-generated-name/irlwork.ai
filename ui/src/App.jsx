  ...
  const handleNegotiate = async (taskId, humanId) => {
    ...
    try {
      ...
      const convRes = await fetch(`${API_URL}/conversations`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: user.token || '' },
        body: JSON.stringify({ agent_id: user.id, task_id: taskId })
      })
      if (!convRes.ok) {
        // Try from agent side — create conversation with human_id
        const convRes2 = await fetch(`${API_URL}/messages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: user.token || '' },
      ...
      const msgRes = await fetch(`${API_URL}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: user.token || '' },
        body: JSON.stringify({ conversation_id: conv.id, content: negotiateMsg.trim() })
      })
      if (msgRes.ok) {
        toast.success('Message sent')
        setNegotiateAppId(null)
