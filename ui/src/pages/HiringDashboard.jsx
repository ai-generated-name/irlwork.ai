        ...
        <Card padding="lg" className="hiring-dash-checklist">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <button onClick={() => onNavigate?.('payments')} style={{
              ...
              background: hasPaymentMethod ? 'rgba(45,122,58,0.09)' : '#F0EAE2',
              borderRadius: 12, border: 'none', cursor: 'pointer', textAlign: 'left', width: '100%'
            }}>
              <div style={{
                width: 28, height: 28, borderRadius: '50%', flexShrink: 0,
                background: hasPaymentMethod ? '#2D7A3A' : '#D1D5DB',
              ...
              <div>
                <p style={{ fontWeight: 500, fontSize: 14, color: hasPaymentMethod ? '#2D7A3A' : 'var(--text-primary)' }}>Add a payment method</p>
            ...
            <button onClick={() => onNavigate?.('create')} style={{
              display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px',
              background: hasPostedTask ? 'rgba(45,122,58,0.09)' : '#F0EAE2',
              borderRadius: 12, border: 'none', cursor: 'pointer', textAlign: 'left', width: '100%'
            }}>
              <div style={{
                width: 28, height: 28, borderRadius: '50%', flexShrink: 0,
                background: hasPostedTask ? '#2D7A3A' : '#D1D5DB',
                display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 14, fontWeight: 700
              }}>{hasPostedTask ? <Check size={14} /> : '2'}</div>
              <div>
                <p style={{ fontWeight: 500, fontSize: 14, color: hasPostedTask ? '#2D7A3A' : 'var(--text-primary)' }}>Post your first task</p>
            ...
            <button onClick={() => { onNavigate?.('browse'); localStorage.setItem('irlwork_has_browsed_humans', 'true') }} style={{
              display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px',
              background: hasBrowsedHumans ? 'rgba(45,122,58,0.09)' : '#F0EAE2',
              borderRadius: 12, border: 'none', cursor: 'pointer', textAlign: 'left', width: '100%'
            }}>
              <div style={{
                width: 28, height: 28, borderRadius: '50%', flexShrink: 0,
                background: hasBrowsedHumans ? '#2D7A3A' : '#D1D5DB',
                display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 14, fontWeight: 700
              }}>{hasBrowsedHumans ? <Check size={14} /> : '3'}</div>
              <div>
                <p style={{ fontWeight: 500, fontSize: 14, color: hasBrowsedHumans ? '#2D7A3A' : 'var(--text-primary)' }}>Browse available humans</p>
            ...
            {safeTasks.slice(0, 5).map(task => (
              <Card key={task.id} padding="sm" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  ...
                  background: task.status === 'open' ? '#F59E0B' : task.status === 'in_progress' ? '#2563EB' : task.status === 'pending_review' ? '#6D4FC2' : '#2D7A3A'
                }} />
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div className="hiring-dash-quick-actions" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
          <Card interactive hoverEffect="shadow" padding="md" onClick={() => onNavigate?.('posted')} className="text-left cursor-pointer">
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#E8764B" strokeWidth="2" style={{ marginBottom: 8 }}>
              <path d="M9 11l3 3L22 4" /><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" />
            </svg>
            <p style={{ fontWeight: 600, fontSize: 14, color: '#1A1A1A', marginBottom: 4 }}>My tasks</p>
            <p style={{ fontSize: 12, color: '#8C8580' }}>View and manage posted tasks</p>
          </Card>
          <Card interactive hoverEffect="shadow" padding="md" onClick={() => onNavigate?.('browse')} className="text-left cursor-pointer">
            ...
            <p style={{ fontSize: 12, color: '#8C8580' }}>Find skilled workers for your tasks</p>
          </Card>
          <Card interactive hoverEffect="shadow" padding="md" onClick={() => onNavigate?.('messages')} className="text-left cursor-pointer">
            ...
            <p style={{ fontSize: 12, color: '#8C8580' }}>Chat with workers and applicants</p>
          </Card>
          <Card interactive hoverEffect="shadow" padding="md" onClick={() => setShowPaymentsExplainer(true)} className="text-left cursor-pointer">
            ...
            <p style={{ fontSize: 12, color: '#8C8580' }}>Learn about escrow and payouts</p>
          </Card>
        </div>
      </div>
    </div>
