  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        <div className="text-[#A69E98]">Loading funnel...</div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#A69E98] mt-0.5">Task lifecycle drop-off from creation to payout, with stage-by-stage conversion rates.</p>
        </div>
      ...
      <Card padding="lg">
              ...
              <div key={stage.key}>
                {i > 0 && stage.rate && (
                  <div className="flex items-center gap-2 py-1.5 pl-4">
                    <ArrowDown size={14} className="text-[#A69E98]" />
                    <span className="text-xs font-medium text-[#A69E98]">{stage.rate} conversion</span>
                  </div>
                )}
        ...
        {/* Overall rate */}
        <div className="mt-6 pt-4 border-t border-[#E8E0D8] flex items-center gap-2">
          <span className="text-sm font-medium text-[#8C8580]">Overall conversion:</span>
          <span className="text-lg font-bold text-orange-600">{rates.overall || '0%'}</span>
          <span className="text-xs text-[#A69E98] ml-2">(created to paid)</span>
        </div>
      </Card>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#E8E0D8]">
                <th className="text-left py-2 text-[#8C8580] font-medium">Transition</th>
                <th className="text-right py-2 text-[#8C8580] font-medium">Rate</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr className="border-t-2 border-[#E8E0D8]">
...
function TimeCard({ label, hours }) {
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-xs text-[#8C8580] mt-1">{label}</p>
    </div>
  )
}
...
function RateRow({ label, rate }) {
  ...
  return (
    <tr className="border-b border-[#FDF6EE]">
      <td className="py-2.5 text-[#1A1A1A]">{label}</td>
      <td className="py-2.5 text-right">
        <span className={`font-medium ${pctNum >= 50 ? 'text-[#2D7A3A]' : pctNum >= 20 ? 'text-[#EAB308]' : 'text-red-500'}`}>
          {rate || '0%'}
        </span>
      </td>
    </tr>
