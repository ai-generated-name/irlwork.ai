...
export const STATUS_COLORS = {
  ...
  paid: 'bg-[#F0EAE2] text-[#8C8580]',
  cancelled: 'bg-[#F0EAE2] text-[#A69E98]',
  expired: 'bg-[#F0EAE2] text-[#A69E98]',
}
...
function durationLabel(task) {
  if (task.deadline) {
    ...
    const days = Math.floor(hours / 24)
    return { text: `Due in ${days}d`, color: 'text-[#A69E98]' }
  }
  if (task.duration_hours) return { text: `~${task.duration_hours}h`, color: 'text-[#A69E98]' }
  if (task.duration) return { text: task.duration, color: 'text-[#A69E98]' }
  return null
}
...
export default function AdminTaskRow({
  ...
  const statusColor = STATUS_COLORS[task.status] || 'bg-[#F0EAE2] text-[#8C8580]'
  ...
  return (
    <div className={`transition-colors duration-1000 ${isNew ? 'bg-orange-50' : 'hover:bg-[#FDF6EE]'}`}>
      {/* Main row */}
      <div className="px-5 py-3.5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Meta line */}
            <p className="text-xs text-[#A69E98] mt-0.5">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="flex items-center gap-1 text-xs text-[#A69E98]" title="Applicants">
              <Users size={13} />
              <span>{task.applicant_count || 0}</span>
            </div>

            <div className="flex items-center gap-1 text-xs text-[#A69E98]" title="Views">
              <Eye size={13} />
              <span>{task.view_count || 0}</span>
            </div>

            <div className="flex items-center gap-1 text-xs text-[#A69E98] w-16 justify-end" title={task.created_at ? new Date(task.created_at).toLocaleString() : ''}>
              <Clock size={13} />
              ...
              <div className="flex items-center gap-1.5 ml-2">
                ...
                {modStatus === 'hidden' && onUnflag && (
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="p-1.5 rounded-lg text-[#2D7A3A] hover:bg-[#F0FDF4] transition-colors disabled:opacity-50"
                    title="Restore task"
                  >
                    <RotateCcw size={15} />
                  </button>
      ...
      {/* Expanded detail */}
      {expanded && (
        <div className="px-5 pb-4 pt-0 border-t border-[#E8E0D8]">
          <div className="pt-3 space-y-3">
            {/* Full description */}
            {task.description && (
              <div>
                <p className="text-xs font-semibold text-[#8C8580] uppercase mb-1">Description</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
              <div>
                <span className="text-[#A69E98]">Task ID</span>
                <div className="flex items-center gap-1 mt-0.5">
                  <code className="text-[#1A1A1A] font-mono text-[11px]">{task.id?.slice(0, 8)}...</code>
                  <button onClick={copyId} className="text-[#A69E98] hover:text-[#8C8580]" title="Copy full ID">
                    <Copy size={11} />
                  </button>
                  {copied && <span className="text-[#2D7A3A] text-[10px]">Copied</span>}
                </div>
              </div>
              {task.agent_email && (
                <div>
                  <span className="text-[#A69E98]">Agent Email</span>
                  <p className="text-[#1A1A1A] mt-0.5">{task.agent_email}</p>
                </div>
              )}
              {task.escrow_status && (
                <div>
                  <span className="text-[#A69E98]">Escrow</span>
                  <p className="text-[#1A1A1A] mt-0.5 capitalize">{task.escrow_status}</p>
                </div>
              )}
              {task.deadline && (
                <div>
                  <span className="text-[#A69E98]">Deadline</span>
