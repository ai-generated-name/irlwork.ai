  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        <div className="text-[#A69E98]">Loading financials...</div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#A69E98] mt-0.5">Revenue breakdown — GMV, platform fees, payouts, refunds, and subscription tiers.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <FinCard
          ...
          bg={data?.disputes?.open > 0 ? 'bg-[#FEF2F2]' : 'bg-[#FDF6EE]'}
          border={data?.disputes?.open > 0 ? 'border-red-200' : 'border-[#E8E0D8]'}
          iconColor={data?.disputes?.open > 0 ? 'text-[#DC2626]' : 'text-[#8C8580]'}
        />
      </div>
      ...
      <Card padding="lg">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            ...
            <p className="text-xs text-[#8C8580]">Monthly recurring revenue</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-[#1A1A1A]">
              {data?.premium_revenue?.active_subscribers || 0}
            </p>
            <p className="text-xs text-[#8C8580]">Active Subscribers</p>
          </div>
          {Object.entries(data?.premium_revenue?.by_tier || {}).map(([tier, count]) => (
            <div key={tier}>
              <p className="text-2xl font-bold text-[#1A1A1A]">{count}</p>
              <p className="text-xs text-[#8C8580]">{tierLabels[tier] || tier} tier</p>
            </div>
          ))}
        </div>
      </Card>
        ...
        <table className="w-full text-sm">
          <tbody>
            ...
            <tr className="border-t-2 border-[#E8E0D8]">
              ...
              <td className="py-3 text-right font-bold text-[#2D7A3A]">
...
function FinCard({ label, value, count, icon, bg, border, iconColor }) {
  return (
    <div className={`rounded-xl border p-5 ${bg} ${border}`}>
      ...
      <p className="text-xs font-medium text-[#8C8580] mt-0.5">{label}</p>
      <p className="text-xs text-[#A69E98] mt-0.5">{count}</p>
    </div>
  )
}
