  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        <div className="text-[rgba(26,20,16,0.28)]">Loading financials...</div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-[#1A1410]">Financial Overview</h2>
          <p className="text-sm text-[rgba(26,20,16,0.28)] mt-0.5">Revenue breakdown — GMV, platform fees, payouts, refunds, and subscription tiers.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <FinCard
          ...
          bg="bg-[#FAFAF8]"
          border="border-[rgba(220,200,180,0.35)]"
          iconColor="text-[rgba(26,20,16,0.50)]"
        />
        <FinCard
          label="Disputes"
          ...
          border={data?.disputes?.open > 0 ? 'border-red-200' : 'border-[rgba(220,200,180,0.35)]'}
          iconColor={data?.disputes?.open > 0 ? 'text-[#DC2626]' : 'text-[rgba(26,20,16,0.50)]'}
        />
      </div>

      {/* Premium / subscription breakdown */}
      <Card padding="lg">
        <h3 className="font-semibold text-[#1A1410] mb-4 text-sm">Premium Subscriptions</h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            <p className="text-2xl font-bold text-[#1A1410]">
              {fmt(data?.premium_revenue?.mrr_cents || 0)}
            </p>
            <p className="text-xs text-[rgba(26,20,16,0.50)]">Monthly recurring revenue</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-[#1A1410]">
              {data?.premium_revenue?.active_subscribers || 0}
            </p>
            <p className="text-xs text-[rgba(26,20,16,0.50)]">Active Subscribers</p>
          </div>
          {Object.entries(data?.premium_revenue?.by_tier || {}).map(([tier, count]) => (
            <div key={tier}>
              <p className="text-2xl font-bold text-[#1A1410]">{count}</p>
              <p className="text-xs text-[rgba(26,20,16,0.50)]">{tierLabels[tier] || tier} tier</p>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card padding="lg">
        <h3 className="font-semibold text-[#1A1410] mb-4 text-sm">Summary</h3>
        <table className="w-full text-sm">
          <tbody>
            ...
            <tr className="border-t-2 border-[rgba(220,200,180,0.35)]">
              <td className="py-3 font-bold text-[#1A1410]">Net revenue (fees + MRR)</td>
              <td className="py-3 text-right font-bold text-[#1A9E6A]">
...
function FinCard({ label, value, count, icon, bg, border, iconColor }) {
  return (
    <div className={`rounded-xl border p-5 ${bg} ${border}`}>
      ...
      <p className="text-xs font-medium text-[rgba(26,20,16,0.50)] mt-0.5">{label}</p>
      <p className="text-xs text-[rgba(26,20,16,0.28)] mt-0.5">{count}</p>
    </div>
  )
}
...
function SummaryRow({ label, value, negative }) {
  return (
    <tr className="border-b border-[#FAFAF8]">
      ...
      <td className={`py-2.5 text-right font-medium ${negative ? 'text-red-500' : 'text-[#1A1410]'}`}>
