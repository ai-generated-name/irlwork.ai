  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-[#7B7672]">Loading disputes...</div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {disputes.length > 0 && (
          <div className="flex gap-1 bg-[#F0EAE2] rounded-xl p-1">
            {[
              { value: 'all', label: 'All' },
      ...
      {error && (
        <div className="bg-[rgba(255, 95, 87, 0.1)] border border-[#FF5F57]/20 rounded-xl p-3 flex items-center justify-between">
          ...
          {disputes.map((dispute) => {
            ...
            return (
              <Card
                key={dispute.id}
                padding="none"
                className="border-2 border-[#E8E0D8] hover:shadow-v4-md transition-shadow"
              >
                {/* Dispute Summary Row */}
                <button
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                        ...
                        <span
                          className={`px-2.5 py-0.5 rounded-[6px] text-xs font-semibold flex-shrink-0 ${
                            isOpen
                              ...
                              : 'bg-[rgba(26,158,106, 0.08)] text-[#2D7A3A]'
                          }`}
                        >
                          {isOpen ? 'Open' : 'Resolved'}
                        </span>
                        {/* Category Badge */}
                        <span className="px-2 py-0.5 rounded-[6px] text-xs font-medium bg-teal/10 text-teal flex-shrink-0">
                          {getCategoryLabel(dispute.category)}
                        </span>
                      </div>

                      <p className="text-sm text-[#7B7672] line-clamp-1">
                        {dispute.reason}
                      </p>

                      <div className="flex items-center gap-3 mt-2 text-xs text-[#B3AFAC]">
                        <span>
                    ...
                    {/* Expand Arrow */}
                    <div className="text-[#B3AFAC] flex-shrink-0 pt-1">
                      <svg
                ...
                {/* Expanded Detail */}
                {isExpanded && (
                  <div className="border-t border-[#E8E0D8] px-4 pb-4 pt-3 space-y-3">
                    {/* Full Reason */}
                    <div>
                      <p className="text-xs font-semibold text-teal tracking-wide mb-1">Reason</p>
                      <p className="text-sm text-[#7B7672]">{dispute.reason}</p>
                    </div>
                        ...
                        <div className="space-y-1">
                          {dispute.evidence_urls.map((url, i) => (
                            /^https?:\/\//i.test(url) ? (
                              ...
                              <span key={i} className="text-sm text-[#A69E98] block truncate">
                                {url} (invalid URL)
                              </span>
                            )
                          ))}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-sm text-[#7B7672]">
                          {filedByName}
                        </p>
                      </div>
                      <div>
                    ...
                    {dispute.status === 'resolved' && dispute.resolution_notes && (
                      <div className="bg-[rgba(26,158,106, 0.08)] border border-[#2D7A3A]/20 rounded-lg p-3">
                        <p className="text-xs font-semibold text-[#2D7A3A] tracking-wide mb-1">Resolution</p>
                        <p className="text-sm text-[#065F46]">{dispute.resolution_notes}</p>
                        {dispute.resolved_at && (
                          <p className="text-xs text-[#2D7A3A]/70 mt-1">
                    ...
                    {isAdmin && isResolving && (
                      <div className="bg-cream border-2 border-teal/20 rounded-xl p-4 space-y-3">
                        ...
                        {/* Resolution Type */}
                        <div>
                          <label className="text-xs font-medium text-[#7B7672] block mb-1.5">
                            Resolution
                          </label>
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all border-2 ${
                                resolution === 'approved'
                                  ? 'border-[#2D7A3A] bg-[rgba(26,158,106, 0.08)] text-[#2D7A3A]'
                                  : 'border-[#E8E0D8] bg-white text-[#7B7672] hover:border-[#2D7A3A]/30'
                              }`}
                            >
                              Approve (Refund)
                            </button>
                            <button
                              ...
                              className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all border-2 ${
                                resolution === 'rejected'
                                  ...
                                  : 'border-[#E8E0D8] bg-white text-[#7B7672] hover:border-[#FF5F57]/30'
                              }`}
                            >
                              Reject
                            </button>
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            rows={3}
                            className="w-full px-3 py-2 border-2 border-[#E8E0D8] rounded-xl text-sm text-[#1A1A1A] placeholder-[#B3AFAC] focus:outline-none focus:border-teal/40 resize-none"
                          />
                        </div>
                          ...
                          <button
                            ...
                            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                              submitting || !resolutionNotes.trim()
                                ? 'bg-[#F0EAE2] text-[#B3AFAC] cursor-not-allowed'
