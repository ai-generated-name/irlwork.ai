...
export default function MessagesTab({
  ...
  const getOnlineStatus = (party) => {
    ...
    return { status: 'offline', label: 'Offline', color: '#A69E98' }
  }
        ...
        <div className={`dashboard-v4-message-thread ${selectedConversation ? '' : 'msg-hide-mobile'}`}>
          {selectedConversation && activeConv ? (
            <>
              ...
              <div style={{ padding: '14px 16px', borderBottom: '1px solid #E8E0D8', display: 'flex', alignItems: 'center', gap: 12, background: 'white', borderRadius: 'var(--radius-lg) var(--radius-lg) 0 0' }}>
                ...
                <div style={{ position: 'relative', flexShrink: 0 }}>
                  ...
                  <span style={{ position: 'absolute', bottom: -1, right: -1, width: 9, height: 9, borderRadius: '50%', background: activeOnline?.color || '#A69E98', border: '2px solid white' }} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <p style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-primary)', margin: 0 }}>{activeOther?.name}</p>
                    <span style={{ fontSize: 11, color: activeOnline?.color || '#A69E98' }}>{activeOnline?.label}</span>
                  </div>
                  {activeConv.task && (
                    <a
