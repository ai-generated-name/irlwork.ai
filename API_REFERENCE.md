...
> **This document reflects the actual implementation** as of the current codebase. It was generated by auditing `api/server.js`, `api/routes/stripe.js`, `api/routes/admin.js`, `api/mcp-server.js`, `api/config/constants.js`, and `api/middleware/adminAuth.js`. Where the implementation differs from `ARCHITECTURE.md`, the inconsistency is noted in Section 10.
...
| `approve_task` | Approve and release payment. Aliases: `release_payment`, `release_escrow` |
| `view_proof` | View proof submissions |
| `dispute_task` | File a dispute |
...
|---|---|---|
| `send_message` | `POST /api/messages` | MCP version **skips email notifications and webhook dispatch**. REST version sends a throttled email (1 per 5 min per conversation) and dispatches a `new_message` webhook via `dispatchWebhook`. MCP version only creates an in-app notification. |
