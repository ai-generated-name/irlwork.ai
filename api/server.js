...
app.get('/api/humans', async (req, res) => {
  ...
  let query = supabase
    .from('users')
    .select('id, name, city, state, hourly_rate, skills, rating, jobs_completed, latitude, longitude, avatar_url')
    .eq('type', 'human')
    .eq('verified', true);
...
app.get('/api/humans/:id', async (req, res, next) => {
  ...
  const { data: user, error } = await supabase
    .from('users')
    .select('id, name, email, city, state, hourly_rate, bio, skills, rating, jobs_completed, profile_completeness, avatar_url')
    .eq('id', req.params.id)
    .eq('type', 'human')
    .single();
...
app.put('/api/humans/profile', async (req, res) => {
  ...
  const { name, wallet_address, hourly_rate, bio, categories, skills, city, latitude, longitude, travel_radius, country, country_code, social_links, avatar_url } = req.body;
...
app.get('/api/humans/directory', async (req, res) => {
  ...
  let query = supabase
    .from('users')
    .select('id, name, city, state, hourly_rate, bio, skills, rating, jobs_completed, verified, availability, created_at, total_ratings_count, social_links, avatar_url')
    .eq('type', 'human')
    .eq('verified', true)
...
app.get('/api/humans/:id/profile', async (req, res) => {
  ...
  const { data: user, error } = await supabase
    ...
    .select(`
      ...
      total_disputes_filed, total_usdc_paid, last_active_at, social_links, avatar_url
    `)
    .eq('id', req.params.id)
    .eq('type', 'human')
    .single();
...
app.put('/api/profile', async (req, res) => {
  ...
  const { name, bio, city, state, hourly_rate, skills, availability, wallet_address, avatar_url } = req.body;
